...
GE.Audio = {
  ...
  sequenceStep()  { this.play(440, 0.1, "sine", 0.08); setTimeout(() => this.play(660, 0.12, "sine", 0.07), 60); },
  gachaReveal(r)  {
...
GE.makeDefaultState = function(profileId) {
  ...
  return {
    ...
    repairLog: [], lastDailyBonus: null, lastDailyPull: null,
  };
};
...
GE.getWrenchEffects = function(tools) {
  return { hasImpact: !!tools.impact_wrench, hasPenOil: !!tools.penetrating_oil, impactInterval: 15 };
};
GE.getPrecisionEffects = function(tools) {
  return { hasTorqueWrench: !!tools.torque_wrench, hasAngleGauge: !!tools.angle_gauge };
};
GE.getBodyworkEffects = function(tools) {
  return { hasDAPolisher: !!tools.da_polisher, hasMediaBlaster: !!tools.media_blaster };
};
// Aliases
GE.getWrenchToolEffects    = GE.getWrenchEffects;
GE.getPrecisionToolEffects = GE.getPrecisionEffects;
GE.getBodyworkToolEffects  = GE.getBodyworkEffects;

// ── SYSTEM COMPLETION BONUS (spec §6: 200-500¥) ──
GE.SYSTEM_COMPLETION_BONUS = {
  engine: 500, fuel: 300, cooling: 250, exhaust: 250,
  drivetrain: 400, brakes: 250, suspension: 300,
  interior: 200, electrical: 300, body: 350, glass: 200, trim: 200,
};
GE.getSystemBonus = function(sysId) { return GE.SYSTEM_COMPLETION_BONUS[sysId] || 200; };

// ── DAILY BONUSES ──
GE.DAILY_BONUS       = 100;
GE.DAILY_LOGIN_BONUS = 100;

// Returns bonus ¥ amount if not yet claimed today, otherwise null
...
GE.calculateQualityMultiplier = function(partTree, vehicleParts) {
  ...
  const maxMult = partTree.rarity === 5 ? 2.0 : 1.5;
...
GE.useDonorPart = function(state, modelId) {
  var ns = JSON.parse(JSON.stringify(state));
  ns.currency.donorParts[modelId] = Math.max(0, (ns.currency.donorParts[modelId]||0)-1);
  return ns;
};
...
GE.sendGift = async function(fromState, toId, partType) {
  ...
  if (window._firebaseReady && window._gameDoc && window._getDoc) {
    try {
      var snap = await window._getDoc(window._gameDoc(toId));
      var r = snap.exists() ? snap.data().payload : null;
      if (r) {
        r.currency = r.currency||{}; r.currency.donorParts = r.currency.donorParts||{};
        r.currency.donorParts[partType] = (r.currency.donorParts[partType]||0)+1;
        await window._setDoc(window._gameDoc(toId),{payload:r,lastModified:Date.now()});
      }
    } catch(e) {}
  }
...
GE.getWrenchFlavor = function(progress) {
  ...
  for (const tier of GE.WRENCH_PROGRESS_FLAVOR) { if (progress >= tier.at) best = tier; }
